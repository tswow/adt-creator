...
class ZoneGroup{
  private:
    ...
    void combine(ZoneGroup& other, int offX=0, int offY=0){ //Combines the two ADT groups.  Should only be called by one of the mash functions
        for(int i=0; i < other.getSize(); i++){
            ...
            temp += std::to_string(other.getAdtX(i) + offX);
            temp += "_";
            temp += std::to_string(other.getAdtY(i) + offY);
            temp += ".adt";
  ...
  public:
    ...
    void write(std::string parentFolder){ //Writes all files from origList to location at adtList
        for(int i=0; i < getSize(); i++){
            char *buffer;
            long size;
            ...
            temp = parentFolder + "_" ;
            temp += std::to_string(getAdtX(i));
            temp += "_";
            temp += std::to_string(getAdtY(i));
            temp += ".adt";
            std::cout << "The temp is " << temp << " for " << getAdtX(i) << " " << getAdtY(i) << "\n";
            ...
            inFile.close();
            outFile.close();
            std::cout << "Writing ADT files to the specified directory\n";
        }
    }
    ...
    void movePos(int offsetX, int offsetY){
        for(int i=0; i < getSize(); i++){
            ...
            std::string cTemp = "";
            temp  = getAdtBase(getSize()-1) + "_";
            temp += std::to_string(getAdtX(i) + offsetX);
            temp += "_";
            temp += std::to_string(getAdtY(i) + offsetY);
            temp += ".adt";
            adtList[i] = temp;
        }
        calcBounds();
